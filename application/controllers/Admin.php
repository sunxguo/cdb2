...
// 	}
	...
	public function couponadd(){
		$supermarkets=$this->getdata->getAllSupermarkets(true,false);
		$buyers = $this->getdata->getAllBuyer(true,false);
		//var_dump($buyers);
		$parameters=array(
			'view'=>'coupon-add',
			'data'=>array('supermarkets'=>$supermarkets,'buyers'=>$buyers)
		);
		$this->adminCommonHandler($parameters);
	}
	...
	public function activityarealist(){
		...
		$amount=$this->getdata->getProducts($productParameters);
		$baseUrl='/admin/activityarealist?placeholder=true';
		$selectUrl='/admin/activityarealist?placeholder=true';
		$currentPage=isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
		$amountPerPage=20;
		$pageInfo=$this->getdata->getPageLink($baseUrl,$selectUrl,$currentPage,$amountPerPage,$amount);
		$productParameters['result']='data';
		// $bannerParameters['limit']=$pageInfo['limit'];
		$products=$this->getdata->getActivitygoods($productParameters);
		$supermarkets=$this->getdata->getAllSupermarkets(true,false);
		if(isset($_GET['sid'])){
			$categories=$this->getdata->getCategories(array(
				'result'=>'data',
	...
	public function recommendlist(){
		...
		$amount=$this->getdata->getProducts($productParameters);
		$baseUrl='/admin/recommendlist?placeholder=true';
		$selectUrl='/admin/recommendlist?placeholder=true';
		$currentPage=isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
		$amountPerPage=20;
		$pageInfo=$this->getdata->getPageLink($baseUrl,$selectUrl,$currentPage,$amountPerPage,$amount);
		$productParameters['result']='data';
		// $bannerParameters['limit']=$pageInfo['limit'];
		$products=$this->getdata->getrecommend($productParameters);
		$supermarkets=$this->getdata->getAllSupermarkets(true,false);
		if(isset($_GET['sid'])){
			$categories=$this->getdata->getCategories(array(
				'result'=>'data',
